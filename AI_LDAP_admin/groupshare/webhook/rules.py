...
def evaluate_rules(
    ...
    requested_admin = bool(user_admin_volume_names(user_groups).intersection(admin_volume_names))
    ...
    for volume in volumes:
        ...
        if not requested_admin and not read_only:
            return False, "Rule C", f"readOnly=false denied for non-admin on volume {volume_name}"

        if not requested_admin and mount_readonly.get(volume_name) is False:
