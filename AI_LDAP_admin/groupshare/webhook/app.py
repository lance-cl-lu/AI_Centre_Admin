...
def load_namespace_policy(namespace: str) -> Dict:
    ...
    annotations = poddefault.get("metadata", {}).get("annotations", {}) or {}
    expected_nfs_server = annotations.get("groupshare.kubeflow.org/nfs-server", "")
    allowed_volume_names = parse_csv(annotations.get("groupshare.kubeflow.org/allowed-volumes", ""))
    allowed_nfs_paths = parse_csv(annotations.get("groupshare.kubeflow.org/allowed-paths", ""))
    admin_volume_names = parse_csv(annotations.get("groupshare.kubeflow.org/admin-groups", ""))
