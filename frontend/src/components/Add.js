...
function Add() {
    ...
    return (
        <div className="AddContent">
            ...
            { matchedResult || permission==='root' ? <> <Card className="AddType"><Card.Body><Link to="/add/user" className="LinkStyle"><span>Add User    <AssignmentIndIcon fontSize='large'/></span></Link></Card.Body></Card> </>:
            null }
            { permission==='root' ? 
            ...
            { permission==='root' ? <>
                ...
                <Card className="AddType"><Card.Body><Link to={SERVICE_EXPORT} className="LinkStyle"><span>Export excel    <DownloadingIcon fontSize="large"/></span></Link></Card.Body></Card></> :
            null }
        </div>
    )
}
